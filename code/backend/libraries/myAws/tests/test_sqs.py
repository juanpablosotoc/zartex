...
from aws.sqs import SQS  # Adjust the import path as necessary
import blossom_exceptions

# Fixtures for mocking boto3 resource
@pytest.fixture
def mock_boto3_resource():
    with patch('aws.sqs.boto3.resource') as mock_resource:
        yield mock_resource
...
def test_post_to_queue_success(mock_boto3_resource):
    ...
    # Assertions
    mock_boto3_resource.assert_called_once_with('sqs')
...
def test_post_to_queue_queue_not_found(mock_boto3_resource):
    ...
    with pytest.raises(blossom_exceptions.SQSServiceError) as exc_info:
    ...
    mock_sqs_resource.get_queue_by_name.side_effect = Exception("Queue does not exist")
    assert f"Error in aws.sqs: Error uploading mess: {message} to sqs queue: {queue_name}Queue does not exist" in str(exc_info.value)
...
def test_post_to_queue_send_message_failure(mock_boto3_resource):
    ...
    mock_sqs_resource.get_queue_by_name.assert_called_once_with(QueueName=queue_name)
    mock_queue.send_message.assert_called_once_with(MessageBody=message)
    assert f"Error in aws.sqs: Error uploading mess: {message} to sqs queue: {queue_name}Failed to send message" in str(exc_info.value)
...
def test_post_to_queue_general_exception(mock_boto3_resource):
    ...
    mock_sqs_resource.get_queue_by_name.assert_called_once_with(QueueName=queue_name)
    mock_queue.send_message.assert_called_once_with(MessageBody=message)
    assert f"Error in aws.sqs: Error uploading mess: {message} to sqs queue: {queue_name}Unexpected error" in str(exc_info.value)
...
def test_post_to_queue_invalid_queue_name(mock_boto3_resource):
    ...
    mock_boto3_resource.assert_called_once_with('sqs')
    mock_sqs_resource.get_queue_by_name.assert_called_once_with(QueueName=queue_name)
    assert f"Error in aws.sqs: Error uploading mess: {message} to sqs queue: {queue_name}Invalid queue name" in str(exc_info.value)
