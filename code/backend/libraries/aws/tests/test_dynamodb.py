...
from aws.dynamodb import DynamoDB  # Adjust the import path as necessary
...
def mock_boto3_resource():
    with patch('aws.dynamodb.boto3.resource') as mock_resource:
        yield mock_resource

@pytest.fixture
def mock_config_aws_region():
    with patch('aws.dynamodb.Config.AWS_REGION', 'us-west-2'):
        yield
...
def test_get_table_exception(mock_boto3_resource, mock_config_aws_region):
    ...
    with pytest.raises(blossom_exceptions.DynamoDBServiceError) as exc_info:
