...
from aws.secrets_manager import SecretsManager  # Adjust the import path as necessary
import blossom_exceptions

# Fixtures for mocking boto3 Session and Config (if needed)
@pytest.fixture
def mock_boto3_session():
    with patch('aws.secrets_manager.boto3.Session') as mock_session:
        yield mock_session
...
def test_get_secret_exception(mock_boto3_session):
    ...
    with pytest.raises(blossom_exceptions.SecretsManagerServiceError) as exc_info:
