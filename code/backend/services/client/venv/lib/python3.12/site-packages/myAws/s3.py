...
class S3:
    ...
    def upload_file(cls, bucket_name: str, object_name: str, file_path: str) -> None:
        ...
        if not os.path.exists(file_path): raise myExceptions.FileNotFoundError(f"File {file_path} not found.")
        try:
            ...
            raise myExceptions.S3ServiceError(f"Error in aws.s3: Error uploading file: {file_path} to bucket: {bucket_name}" + str(e))
    ...
    def download_file(cls, bucket_name: str, object_name: str, file_path: str) -> None:
        ...
        if os.path.exists(file_path): raise myExceptions.FileAlreadyExistsError(f"File {file_path} already exists.")
        try:
            client = cls.get_s3_client()
            ...
            raise myExceptions.S3ServiceError(f"Error in aws.s3: Error downloading file: {object_name} from bucket: {bucket_name}" + str(e))
